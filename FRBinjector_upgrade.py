...
def main():
    ...
    [nsamps, nchans] = np.shape(fb.get_spectra(Binsert-opts.width,Binsert+binsweep+opts.width))
    #outputfb = opts.inputfb.split('/')[-1].split('.fil')[0]+"_FRB.fil"
    outputfb = os.path.splitext(opts.inputfb)[0] + "_FRB" + os.path.splitext(opts.inputfb)[1]
    #print "nsamps, nchans"
    #print nsamps, nchans
    fb_spec = np.copy(fb.get_spectra(Binsert-opts.width,Binsert+binsweep+opts.width))
    #print fb_spec
    fb_spec_T = np.transpose(fb_spec)
    #print "shape"
    #print np.shape(fb_spec)
    ...
    for i in range(nchans):
        ...
        if opts.onebit:
            ...
            #print fb_spec_T[i,:][tempbin:opts.width]
            print "np shape spec[tempbin:tempbin+opts.width]"
            print np.shape(fb_spec_T[i,:][tempbin:tempbin+opts.width])
            #print fb_spec_T[i,:]#[tempbin-opts.width/2:tempbin+opts.width/2]
            print "np shape spec[i,:]"
            print np.shape(fb_spec_T[i,:])
            np.add(fb_spec_T[i,:][tempbin:tempbin+opts.width], gauss/nchans)
            #fb_spec_T[i,:][tempbin-opts.width/2:tempbin+opts.width/2] += gauss/nchans
            #np.add(fb_spec_T[i,:][tempbin-opts.width/2:tempbin+opts.width/2],gauss/nchans)
