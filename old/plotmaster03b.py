...
def gathersps2(INDIR,OUTDIR):
    ...
    if sp_files:
        ...
        os.system("tail -n+2 -q " + INDIR + "*.singlepulse >> " + OUTDIR + "allsps._MEGAtxt")
    else:
        open(OUTDIR + "allsps.txt",'w').close()
...
def plotstuff(TXTDIR,FIL):
    ...
    m_I_T = np.sqrt(nchans)/7. # m_I_T = sqrt(Nchan)/SN_min
    ...
    if the_good:
        smalldat = np.loadtxt(os.path.join(TXTDIR,"EventSummary.txt"))
        if np.ndim(smalldat)==1: smalldat = np.zeros((1,12))
        SNvals = smalldat[:,1]
        DMvals = smalldat[:,0]
        Tvals = smalldat[:,2]
    ...
    if the_bad:
        bigdat = np.loadtxt(os.path.join(TXTDIR,"allsps.txt"))
        if np.ndim(bigdat)==1: bigdat = np.zeros((1,12))
        bigDM = bigdat[:,0]
        bigSN = bigdat[:,1]
        bigT = bigdat[:,2]
    ...
    if the_ugly:
        ...
        if np.ndim(megadat)==1: megadat = np.zeros((1,12))
        megaDM = megadat[:,0]
        megaSN = megadat[:,1]
        megaT = megadat[:,2]
    ...
    for i in range(4):
        ...
        if any(tmpT): ax5.scatter(tmpT,tmpDM,color='blue',linewidth=.5,facecolor='none',s=(tmpSN-7)**2.+1)
