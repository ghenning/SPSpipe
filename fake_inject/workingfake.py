...
#def main(FIL,TIME,DM,WIDTH,FWHM,SN,ONEBIT):
def main(FIL,TIME,DM,WIDTH,SN,ONEBIT):
    fb = filterbank.FilterbankFile(FIL)
    totsamps = fb.N
    ...
    tdelay = dispdelay(DM,freqlo,freqhi)
    binsweep = int(tdelay/fb.dt)
    fwhm = int(np.ceil(WIDTH/fb.dt))
    width = fwhm*4
    #width = int(np.ceil(WIDTH/fb.dt))
    #if opts.onebit: fwhm = int(np.ceil(opts.width/fb.dt))
    ###peak_center = int(np.ceil(.5*WIDTH/fb.dt))
    peak_center = width/2
    #peak_center = int(np.ceil(opts.b/fb.dt))
    Binsert = int(TIME/fb.dt) + width/2
    '''if opts.onebit:
        tdelay *= 8
        binsweep *= 8
        width *= 8
        ...
        peak_center *= 8
        Binsert *= 8'''
    ##############3print "freqhi %f freqlo %f " % (freqhi,freqlo)
    ...
    outputfb = os.path.splitext(FIL)[0] + "_FRB" + os.path.splitext(FIL)[1]
    if ONEBIT:
    ...
    while sampread:
        ...
        if (startbin <= Binsert < endbin):
            ...
            if endbin - Binsert < binsweep:
                if ONEBIT: endbin += mod_eight(binsweep)
            ...
            #blabla = np.median(spec,axis=0)
            blabla = np.median(spec)
            print "blabla: %f" % blabla
            #scale_fac = opts.SN*RMS/nchans
            ###scale_fac = opts.SN*RMS
            ####scale_fac = SN*RMS/(np.sqrt(channels)*np.sqrt(width))
            ####scale_fac = SN*RMS/(np.sqrt(channels))
            ########scale_fac = SN/RMS/np.sqrt(channels)
            #scale_fac = SN*RMS/(np.sqrt(channels)*np.sqrt(width))
            scale_fac = SN*blabla/(np.sqrt(channels)*np.sqrt(fwhm))
            ####scale_fac = SN*RMS/np.sqrt(channels)
            ############# CHECKING #####################
            print "startbin"
            print startbin
            ...
            for i in range(nchans):
                tempdelay = dispdelay(DM,topdownfreqs[i],freqhi)
                ...
                if ONEBIT:
                    ...
                    if i==511:
                        ...
                        print gauss
                        print "data"
                        print spec_T[i,:][tempbin:tempbin+width]
                        #plt.plot(gauss)
                        #plt.plot(spec_T[i,:][tempbin:tempbin+width])
                        #plt.show()
...
if __name__=='__main__':
    ...
    main(opts.inputfb,opts.Tinsert,opts.DM,opts.width,opts.SN,opts.onebit)
